    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 sceneUVs = (i.projPos.xy / i.projPos.w) + lerp(((_subtractor1.rg*(_Refraction*0.2))*_multiplier1),float2(0,0),saturate(pow((distance(i.posWorld.rgb,_WorldSpaceCameraPos)/_RefractionDistance),_RefractionFalloff)));
...
/////// Diffuse:
                ...
                float _value1 = 1.0;
                float3 _blend1 = saturate((_DeepWaterColor.rgb+(sceneColor.rgb*pow(saturate((_value1 + ( ((max(0, LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sceneUVs)) - _ProjectionParams.g)-partZ) - _vector31) * (0.5 - _value1) ) / (((_MainColor.rgb*(10.0/_Density))+0.0) - _vector31))),_Fade))));
                float2 _mask1 = _subtractor1.rg;
                float2 _remap1 = (((sceneUVs * 2 - 1).rg+(float2(_mask1.r,_mask1.g)*_Distortion))*0.5+0.5);
